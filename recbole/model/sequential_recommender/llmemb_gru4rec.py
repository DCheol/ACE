...
class LLMemb_GRU4Rec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        super(LLMemb_GRU4Rec, self).__init__(config, dataset)
        ...
        reduced_llm_item_emb = np.concatenate((np.zeros((1, 1536)), reduced_llm_item_emb), axis=0)  # zero padding for [PAD] token
    ...
    def _init_weights(self, module):
        if isinstance(module, (nn.Embedding)) and module is not self.item_embedding and module is not self.PCA_item_embedding and module is not self.llm_item_embedding:
