...
class WhitenRecPlus_BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(WhitenRecPlus_BERT4Rec, self).__init__(config, dataset)
        ...
        self.item_embedding = WhitenRecPlus_Item_Embedding(config, mapped_feat, self.n_items + 1, self.hidden_size, scale=None, padding_idx=0, )
...
class WhitenRecPlus_Item_Embedding(torch.nn.Module):
    def __init__(self, config, F, item_num, hidden_size, scale=1, padding_idx=0, ):
        ...
        X_full[1:-1] = torch.from_numpy(ZCA_full).float()
        X_group[1:-1] = torch.from_numpy(ZCA_group).float()
