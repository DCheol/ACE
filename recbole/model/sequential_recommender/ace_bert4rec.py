...
class ACE_BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(ACE_BERT4Rec, self).__init__(config, dataset)
        ...
        # self.item_embedding = ACE_Item_Embedding(config, mapped_feat, self.n_items + 1, self.hidden_size, scale=None, padding_idx=0, )
        self.item_embedding = ACE_Item_Embedding(config, mapped_feat, self.n_items+1, self.hidden_size, scale=config['scale'], padding_idx=0,)
...
class ACE_Item_Embedding(torch.nn.Module):
    def __init__(self, config, F, item_num, hidden_size, scale=1, padding_idx=0, ):
        ...
        # F = F[1:]
        # norms = np.linalg.norm(F[1:], axis=1, keepdims=True)
        # F[1:] = F[1:] / norms
        ...
        if not os.path.exists(f'./tmp/{self.dataset_name}_F_U.npy'):
            F_items = F[1:-1]
            U, S, Vt = np.linalg.svd(F_items, full_matrices=False)
        ...
        # U, S, Vt = np.linalg.svd(F[1:-1], full_matrices=False)
        ...
        X_full[1:-1] = torch.from_numpy(X_scaled).float()
        with torch.no_grad():
