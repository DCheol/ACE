...
class Alphafuse_BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(Alphafuse_BERT4Rec, self).__init__(config, dataset)
        ...
        self.item_embedding = Item_Embedding(mapped_feat, self.n_items, self.hidden_size, init_type=self.init_type, ID_dim=int(self.hidden_size/2), scale=None, padding_idx=0)
...
class Item_Embedding(torch.nn.Module):
    def __init__(self, language_embs, item_num, hidden_size, init_type="uniform", ID_dim=64, scale=None, padding_idx=0):
        ...
        ### init LLM embeddings ###
        ### init LLM embeddings ###
        language_embs = language_embs[1:-1,:] * cal_scale
        ...
        clipped_language_embs = np.vstack([padding_emb, clipped_language_embs, padding_emb]) # (self.item_num, 128)
