...
class LLMemb_BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(LLMemb_BERT4Rec, self).__init__(config, dataset)
        ...
        mapped_feat = np.zeros((self.n_items+1, loaded_feat.shape[1]), dtype=np.float32)
        ...
        if config["target_std"] is not None:
            ...
            mapped_feat[1:-1,:] = (mapped_feat[1:-1,:] - mapped_feat[1:-1,:].mean()) / (mapped_feat[1:-1,:].std()+1e-8) * self.target_std

        pca = PCA(n_components=1536)
        reduced_llm_item_emb = pca.fit_transform(mapped_feat[1:-1,:])
        reduced_llm_item_emb = np.concatenate((np.zeros((1, 1536)), reduced_llm_item_emb, np.zeros((1, 1536))), axis=0)  # zero padding for [PAD] token
        
        self.PCA_item_embedding = nn.Embedding.from_pretrained(torch.Tensor(reduced_llm_item_emb))
        self.PCA_item_embedding.weight.requires_grad = False  # allow fine-tuning of LLM embeddings
        ...
        self.adapter = nn.Sequential(
            nn.Linear(1536, int(1536 / 2)),
            nn.Linear(int(1536 / 2), self.hidden_size))
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)) and module is not self.item_embedding and module is not self.PCA_item_embedding and module is not self.llm_item_embedding:
    ...
    def item_embedding(self, item_seq):
        item_emb = self.PCA_item_embedding(item_seq)
    ...
    def reg_loss(self, target, user_emb):
        ...
        K = idx.size(0)
        if K == 0:
            return user_emb.new_zeros(())
        b_idx, l_idx = idx[:, 0], idx[:, 1]
        sel_user = user_emb[b_idx, l_idx]
        sel_item_ids = target[b_idx, l_idx].long()

        llm_feat = self.llm_item_embedding(sel_item_ids)
        teacher_embs = self.llm_item_embedding.weight[:-1]
        ...
        sim_student = torch.matmul(sel_user, student_embs.transpose(0, 1))
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "BPR":
            ...
            test_item_emb = self.item_embedding(item_ids)  # [item_num H]
            logits = (
