...
class WhitenRec_BERT4Rec(SequentialRecommender):
    def __init__(self, config, dataset):
        super(WhitenRec_BERT4Rec, self).__init__(config, dataset)
        ...
        self.item_embedding = WhitenRec_Item_Embedding(config, mapped_feat, self.n_items + 1, self.hidden_size, scale=None, padding_idx=0, )
...
class WhitenRec_Item_Embedding(torch.nn.Module):
    def __init__(self, config, F, item_num, hidden_size, scale=1, padding_idx=0, ):
        ...
        if os.path.exists(f'./tmp/{config["dataset"]}_F_centerized_S.npy'):
            print("Load precomputed ZCA matrix")
            U = np.load(f'./tmp/{config["dataset"]}_F_centerized_U.npy')
            S = np.load(f'./tmp/{config["dataset"]}_F_centerized_S.npy')
            Vt = np.load(f'./tmp/{config["dataset"]}_F_centerized_Vt.npy')
        else:
            print("Compute ZCA matrix")
            U, S, Vt = np.linalg.svd(F, full_matrices=False)
            os.makedirs('./tmp/', exist_ok=True)
            np.save(f'./tmp/{config["dataset"]}_F_centerized_U.npy', U)
            np.save(f'./tmp/{config["dataset"]}_F_centerized_S.npy', S)
            np.save(f'./tmp/{config["dataset"]}_F_centerized_Vt.npy', Vt)
        # U, S, Vt = np.linalg.svd(F, full_matrices=False)
